...
router.post('/update-user', auth.ensureSignedIn, auth.currentUser,async (req, res) => {
    const { currentUser } = req;
    const user = await userService.update(currentUser?._id);
    res.json(user);
})

router.post('/update-password', auth.ensureSignedIn, auth.currentUser, async (req, res) => {
    const result = await userService.updatePass(req.body);
    res.json(result);
})

router.post('/delete-user', auth.ensureSignedIn, auth.currentUser, async (req, res) => {
    const { currentUser } = req;
    const user = await userService.remove(currentUser?._id);
    res.json(user);
})
